#!/usr/bin/env bash
# =============================================================
#   QuantumARK v3.0 — Script de Instalación de Dependencias
...
print_header() {
  echo ""
  echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}${BOLD}║       QuantumARK v3.0 — Setup Script        ║${NC}"
  echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════════╝${NC}"
  echo ""
}
...
setup_macos() {
  ...
  ok "macOS listo para compilar QuantumARK"
}
...
setup_debian() {
  ...
  ok "Ubuntu/Debian listo para compilar QuantumARK"
}
...
setup_fedora() {
  ...
  ok "Fedora/RHEL listo para compilar QuantumARK"
}
...
setup_arch() {
  ...
  ok "Arch Linux listo para compilar QuantumARK"
}

# ── Compilar QuantumARK ──────────────────────────────────────
compile_app() {
  step "Compilando QuantumARK v3.0 en modo release..."
  ...
  BINARY="target/release/quantumark-tauri"
  if [[ -f "$BINARY" ]]; then
    echo ""
    info "Binario generado: $(pwd)/$BINARY"
    info "Tamaño: $(du -sh $BINARY | cut -f1)"
    echo ""
    echo -e "${GREEN}${BOLD}🎉 QuantumARK v3.0 compilado con éxito.${NC}"
    echo -e "   Para ejecutar:"
    echo -e "   ${CYAN}./target/release/quantumark-tauri${NC}"
  fi
}
...
echo ""
read -rp "  ¿Compilar QuantumARK ahora? [s/N]: " COMPILE
if [[ "$COMPILE" =~ ^[sS]$ ]]; then
  compile_app
else
  echo ""
