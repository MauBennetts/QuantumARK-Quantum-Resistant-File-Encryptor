use ml_kem::{MlKem1024, EncapsulationKey, DecapsulationKey, KeyExport, Encapsulate, Decapsulate};
...
/// Genera un par de llaves post-cuánticas (ML-KEM-1024 y Dilithium5).
/// Retorna (public_key_json_bytes, private_key_json_bytes).
pub fn generate_pqc_keypair() -> Result<(Vec<u8>, Vec<u8>), String> {
    // Generar claves KEM (ML-KEM-1024)
    let mut seed = [0u8; 64];
    rand::RngCore::fill_bytes(&mut rand::rngs::OsRng, &mut seed);
    let kem_sec = DecapsulationKey::<MlKem1024>::from_seed(seed.into());
    let kem_pub = kem_sec.encapsulation_key();

    // Generar claves de Firma (Dilithium5)
    let (sig_pub, sig_sec) = dilithium::keypair();

    let public_key_data = PQCKeyPairHex {
        kem_public: hex::encode(kem_pub.to_bytes().as_slice()),
    ...
    let private_key_data = PQCPrivateKeyHex {
        kem_private: hex::encode(kem_sec.to_bytes().as_slice()),
...
/// Encapsula un secreto compartido usando ML-KEM-1024 a partir de los bytes de clave pública JSON.
...
pub fn encapsulate_secret(public_key_json: &[u8]) -> Result<(Vec<u8>, Vec<u8>), String> {
    ...
    let key_array: [u8; 1568] = kem_pub_bytes.as_slice().try_into()
        .map_err(|_| "Longitud de llave pública ML-KEM-1024 inválida".to_string())?;

    let key_struct = Array::from(key_array);
    let pubkey = EncapsulationKey::<MlKem1024>::new(&key_struct)
        .map_err(|e| format!("Llave KEM ML-KEM-1024 inválida: {:?}", e))?;

    let (ct, ss) = pubkey.encapsulate_with_rng(&mut OsRng10);
...
/// Desencapsula un secreto compartido usando ML-KEM-1024 a partir de los bytes de clave privada JSON.
pub fn decapsulate_secret(private_key_json: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let seed_array: [u8; 64] = kem_priv_bytes.as_slice().try_into()
        .map_err(|_| "Longitud de semilla de llave privada ML-KEM-1024 inválida".to_string())?;

    let seckey = DecapsulationKey::<MlKem1024>::from_seed(seed_array.into());

    let ct_array: [u8; 1568] = ciphertext.try_into()
        .map_err(|_| "Longitud de ciphertext ML-KEM-1024 inválida".to_string())?;

    let ct = Array::from(ct_array);

    let ss = seckey.decapsulate(&ct);
